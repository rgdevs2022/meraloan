  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 flex items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16 lg:pt-36 lg:pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
          ...
          <div className="text-white">
            ...
            >
              Get the{" "}
              <span className="text-[#FF6B6B]">Best Loan</span>{" "}
              Offers in Minutes
            </motion.h1>
            ...
            >
              ...
              <Button
                ...
                className="border-white/30 text-white bg-white/10 hover:bg-white/20 font-semibold px-8 py-4 h-auto rounded-full text-base backdrop-blur-sm transition-all duration-200"
              >
      ...
      <motion.div
        ...
        className="relative bg-white/5 border-t border-white/10 backdrop-blur-sm"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {TRUST_STATS.map(({ icon: Icon, value, label }) => (
              <div key={label} className="flex items-center gap-3 text-white">
                <div className="w-10 h-10 bg-[#E53935]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                  <Icon className="h-5 w-5 text-[#FF6B6B]" />
                </div>
                <div>
                  <div className="text-lg font-black">{value}</div>
                  <div className="text-xs text-white/60 leading-tight">{label}</div>
                </div>
              </div>
            ))}
          </div>
...
function DashboardIllustration() {
  return (
    <div className="relative w-full max-w-lg mx-auto">
      {/* Main dashboard card */}
      <div className="rounded-3xl p-6 shadow-2xl backdrop-blur-xl bg-gradient-to-br from-white/[0.12] to-white/[0.04] border border-white/20">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-white/60 text-xs mb-1">CREDIT SCORE</p>
        ...
        <div className="bg-white/[0.08] rounded-xl p-3 border border-white/10">
          <p className="text-white/60 text-xs mb-2">TOP RECOMMENDED BANKS</p>
...
function MetricCard({ label, value, positive }: { label: string; value: string; positive?: boolean }) {
  return (
    <div className="bg-white/[0.08] border border-white/10 rounded-xl p-3">
      <p className="text-white/50 text-xs mb-0.5">{label}</p>
      <p className={`text-sm font-bold ${positive ? "text-[#4ADE80]" : "text-white"}`}>{value}</p>
    </div>
  );
}
